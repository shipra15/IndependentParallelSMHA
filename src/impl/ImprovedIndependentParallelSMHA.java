...
public class ImprovedIndependentParallelSMHA {
...
//	AtomicBoolean anchorUpdateInProgress = new AtomicBoolean();
	...
	public void ImprovedIndependentParallelSMHA(State startState, List<State> goalStates) {
...
//			System.out.println(heuristic);
	...
	private void smha(Integer heuristic, Double minKeyAnchor) {
		...
		while(!queue.isEmpty()) {
			...
			Double minKeyInadmissible = anchorKey(minKeyNode);
			if(minKeyInadmissible <= Constants.w2*minKeyAnchor)
			{
				queue.remove();
				expandNode(minKeyNode, heuristic);
				System.out.println("inadmissible expansion");
			}
			else 
			{
				startAnchorUpdate(heuristic);
				System.out.println("anchor expansion");
			}
		}
		System.out.println("queue "+heuristic+" empty");
	}
...
//			printWriter.flush();
	...
	public static void main(String args[]) {
		...
		new ImprovedIndependentParallelSMHA().ImprovedIndependentParallelSMHA(startState, randomGoalStates);
	}
